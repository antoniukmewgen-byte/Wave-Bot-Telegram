...
async def _handle_lead_event(event: dict):
    ...
    if distributed_row:
        ...
        if str(pipeline_id) == AMO_PIPELINE_ID and str(status_id) == AMO_HOT_STATUS_ID:
            # Лід досі в нашій гарячій воронці, У ТОМУ Ж САМОМУ статусі
            # "Кваліфікація" — сюди потрапляє як "відлуння" від PATCH, яким
            # бот сам щойно виставив відповідального (менеджер натиснув "Беру
            # в роботу"), так і справжня ситуація, коли відповідального
            # руками змінили в CRM. pipeline/status в обох випадках однакові,
            # тому розрізняємо за ФАКТИЧНИМ відповідальним: якщо в Kommo
            # відповідальний — той самий менеджер, що вже трекається як
            # "розподілений", це наше власне відлуння і нічого робити не
            # треба; якщо відповідальний інший — заявку справді передали
            # комусь ще, повертаємо старого менеджера в чергу.
            #
            # ВАЖЛИВО: перевіряти тут ЛИШЕ pipeline_id (без status_id) — баг:
            # системні статуси "Закрито успішно"/"Закрито і не реалізовано"
            # (142/143) теж лежать у цій самій воронці (AMO_PIPELINE_ID), і
            # відповідальний при закритті менеджером зазвичай НЕ змінюється —
            # без перевірки status_id таке закриття помилково розпізнавалось
            # як "відлуння", і менеджера не звільняли з "на зв'язку з клієнтом".
